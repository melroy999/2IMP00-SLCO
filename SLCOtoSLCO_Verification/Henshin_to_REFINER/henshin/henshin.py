...
class ConditionalUnit(Unit):
    ...
    def __init__(self, cu_if=None, then=None, cu_else=None, **kwargs):
        super().__init__(**kwargs)
        if cu_if is not None:
            self.cu_if = cu_if
        if cu_then is not None:
            self.cu_then = cu_then
        if cu_else is not None:
            self.cu_else = cu_else
