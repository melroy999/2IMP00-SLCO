...
def mcrl2_sourcestate_accesspattern(s, o, sm, b):
	...
	for tr in trans[o.type][sm][state]:
		...
		if b:
			...
			access = statement_condition_access[o][st]
		accesslist[0] |= access[0]
		accesslist[1] |= access[1]
	output = "A'("
	sorted_access = sorted(list(accesslist[0]))
	# if the list requires dynamic sorting (it contains more than one dynamic accesses to an array), indicate this
	occurrences = set([])
	found = False
	for v in sorted_access:
	...
	sorted_access = sorted(list(accesslist[1]))
	occurrences = set([])
	found = False
	for v in sorted_access:
		v_splitted = v.split("(index\'(")
